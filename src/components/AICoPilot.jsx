...
export default function AICoPilot() {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem("anthropic_api_key") || import.meta.env.VITE_ANTHROPIC_API_KEY || '');
...
OUTPUT FORMAT — return ONLY this JSON:
{
  ...
  const callClaude = async (userMsg, systemMsg) => {
    if (!apiKey.trim()) throw new Error("No API key — add your Anthropic API key in the Settings tab");
    ...
    if (!text) throw new Error(`Empty response (HTTP ${res.status}) — check your API key in Settings`);
    let data;
    try { data = JSON.parse(text); }
    catch { throw new Error(`Bad response (HTTP ${res.status}): ${text.slice(0, 200)}`); }
      ...
      {activeTab === "settings" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ padding: "12px 14px", border: "1px solid var(--color-border-tertiary)", borderRadius: 10 }}>
            <p style={{ margin: "0 0 6px", fontSize: 12, fontWeight: 500 }}>Anthropic API key</p>
            <p style={{ margin: "0 0 8px", fontSize: 11, color: "var(--color-text-secondary)" }}>Get one free at console.anthropic.com — saved locally in your browser only.</p>
            <input
              type="password"
              value={apiKey}
              onChange={e => { setApiKey(e.target.value); localStorage.setItem("anthropic_api_key", e.target.value); }}
              placeholder="sk-ant-..."
              style={{ width: "100%", fontSize: 12, padding: "6px 10px", boxSizing: "border-box", fontFamily: "monospace" }}
            />
            {apiKey && <p style={{ margin: "6px 0 0", fontSize: 11, color: "#1D9E75" }}>Key saved.</p>}
          </div>
